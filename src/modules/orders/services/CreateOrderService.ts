...
class CreateOrderService {
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const existsProductIds = existsProduct.map(product => product.id);
    ...
    const quantityAvailable = products.filter(
      product =>
        existsProduct.filter(productExist => productExist.id == product.id)[0]
          .quantity < product.quantity,
    );

    if (quantityAvailable.length) {
      throw new AppError(
        `The quantity ${quantityAvailable[0].id}
